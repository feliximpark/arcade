...
var fieldX =  202; // plus 101 für nächste Feld
var fieldY = 404; // Start bei 60, plus 83 für nächstes Feld
var key = null; // Variable, um den Tastaturbefehl zu speichern
var bluegem = true;
var allEnemies = [];
var allGreenGems = [];
var allBlueGems = [];
...
Enemy.prototype.update = function(dt) {
    ...
    if (this.x >= 505) {
        this.speed = Math.floor((Math.random() * (6 - 3) + 3));
        this.x = -80;
        console.log ("Bugs sind bei x = 505 plus");
    } else {
...
// Creating new objects with the object-creator-functions
...
// Creating an array of all items which to iterate over it in the function checkCollisionsItems
allItems.push(gem_green, gem_blue, rock, heart);
...
Enemy.prototype.checkCollisions = function(){
    ...
    for (i=0; i<=allEnemies.length-1; i++) {


        if (allEnemies[i].y === (player.y-12) && allEnemies[i].x > player.x-75 && allEnemies[i].x<player.x+70){
            console.log ("Player versenkt");
            player.live -= 1;
            console.log("checkCollision meldet: " + player.live);
            document.getElementById("life").innerHTML = player.live;
            player.x = fieldX;
            player.y = fieldY;
...
Gems.prototype.checkCollisionsItems = function(){

    for (i=0; i<allItems.length; i++) {
        if (allItems[i].y === player.y+48 && allItems[i].x === player.x+20) {
            console.log("Player trifft auf Gem");
